...
final class GauntletDecisionStateTests: XCTestCase {
    ...
    func testWinningLastChanceGradeShowsItsResultBeforeOpeningTheShop() throws {
        ...
        let result = try XCTUnwrap(state.grade(showcaseIndex: 0, deferResolution: true))
        XCTAssertEqual(result.grade, 9)
        XCTAssertEqual(result.fee, 2, accuracy: 1e-9)
        XCTAssertEqual(result.oldValue, 12.30, accuracy: 1e-9)
        XCTAssertEqual(result.newValue, 24.60, accuracy: 1e-9)
    ...
    func testFailedLastChanceGradeShowsItsResultThenEndsWithoutADeadEnd() throws {
        ...
        try settleLastPack(state)
        let result = try XCTUnwrap(state.grade(showcaseIndex: 0, deferResolution: true))
        XCTAssertEqual(result.grade, 7)
        XCTAssertLessThan(result.newValue, result.oldValue)
        XCTAssertEqual(state.phase, .ripping)
        state.finishGrading()
    ...
    func testReloadDuringGradeReviewResolvesTheSavedResultWithoutRerolling() throws {
        let state = seed(DebugGauntletScenario.lastPack.snapshot)
        try settleLastPack(state)
        XCTAssertEqual(state.grade(showcaseIndex: 0, deferResolution: true)?.grade, 9)
