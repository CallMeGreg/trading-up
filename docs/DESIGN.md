...
can be swapped for commissioned art later without changing the game. The app icon is
built from the same code, so the two can't drift apart.
...
2. **Art direction** — ✅ *decided:* hand-built flat-vector creatures, one per card,
   generated by `tools/generate_art.py` (see §1), plus a per-set code-drawn pack
   landscape in `SetArt.swift`. The earlier procedural "sigil" emblems are
   retired; `SigilView` survives only as the fallback if an art asset is ever
   missing.
